...
function findComponentStates(domElement: HTMLElement): any[] {
  ...
  if (!fiberKey) {
    console.error("Could not find Fiber node.");
    return [];
  }
  ...
  // 1. Walk up to find the component Fiber that owns the state
  while (current) {
    ...
    if (!current.return) break; // Reached the root

    current = current.return;
  }

  if (!componentFiber) {
    console.log("Could not find a component with state.");
    return [];
  }

  console.log("Found component Fiber:", componentFiber.type);

  // 2. Iterate through the hooks linked list
  const states = [];
  let hook = componentFiber.memoizedState;

  while (hook) {
    // A useState hook has a 'queue' property, which is a good identifier.
    // Effect hooks (useEffect, etc.) do not have this.
    if (hook.queue) {
      states.push(hook.memoizedState);
    }
    hook = hook.next; // Move to the next hook in the chain
  }
...
console.log("run getData() to extract data");
...
const getData = () => {
  ...
  if (!dailyMetricsElement) throw new Error("Daily metrics element not found");
  ...
  if (!detailedSelectElement)
    throw new Error("Detailed select element not found");
